...
def create_cumulative_csv(data):
    ...
    for month_and_week in data["month and week"].unique():
        ...
        top_performers = df_month_and_week.sort_values(
            by="avg gain/loss ratio", ascending=False
        ).head(3)
        ...
        df.loc[month_and_week, "top performer 1 gain/loss ratio"] = top_performers.iloc[
            0
        ]["avg gain/loss ratio"]
        df.loc[month_and_week, "top performer 2 gain/loss ratio"] = top_performers.iloc[
            1
        ]["avg gain/loss ratio"]
        df.loc[month_and_week, "top performer 3 gain/loss ratio"] = top_performers.iloc[
            2
        ]["avg gain/loss ratio"]
...
def get_current_month_top_performers():
    ...
    top_performers = (
        current_month_and_week_data.sort_values(
            by="avg gain/loss ratio", ascending=False
        )
        .head(3)[["name", "avg gain/loss ratio"]]
        .values.tolist()[0:3]
    )
